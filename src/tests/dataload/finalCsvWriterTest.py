...
def countriesCsvTest(fileInfo):
    countryCsv = pd.read_csv(join(fileInfo.outputDir, 'countries.csv'))
    ...
    assert len(countryCsv.index) ==  25, 'CsvWriter: countries.csv | Data is either missing or more than expected'

def indicatorsCsvTest(fileInfo):
    indicatorsCsv = pd.read_csv(join(fileInfo.outputDir, 'indicators.csv'))
    ...
    assert len(indicatorsCsv.index) ==  55, 'CsvWriter: indicators.csv | Data is either missing or more than expected'
...
def statsCsvTest(fileInfo):
    ...
    # load the stats csv
    statsCsv = pd.read_csv(join(fileInfo.outputDir, 'stats.csv'))
    ...
    assert len(statsCsv.index) ==  1525, 'CsvWriter: stats.csv | Data is either missing or more than expected'